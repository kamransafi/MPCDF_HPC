...
**`--nodes`**: is the number of nodes, if it is and array job, you can set to several nodes, but only if you request at least half of the node, i.e. 36 CPUs per task. Take into account that if you request all memory or CPUs of the nodes the waiting time will probaly increase and slow down the process. 
